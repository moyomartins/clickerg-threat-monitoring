...
export function applyManualDecision(visitor: Visitor, status: VisitorStatus, at: number): Visitor {
  ...
  return {
    ...
    exclusionEvents: status === 'blocked' ? [...new Set(visitor.visits.flatMap(v => v.platform ? [v.platform] : []))].map(platform => ({ platform, requestedAt: at, scope: 'Mock advertising account' })) : [],
  };
}
