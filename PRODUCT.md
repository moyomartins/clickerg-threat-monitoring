...
- Two levels: a **visitor list** — in Replay, a grid of page replays rather than a table, each card carrying the visitor's most recent reporting arrival plus status, IP, location, visit count, paid mix, spend and the one-line reason — and a **visitor detail** (a hero replay, the verdict, then every arrival replayed in a horizontally scrolling strip with per-arrival signals and running confidence, and the plain-language notes stacked beneath).
- **Sorting is an explicit control, not a table header.** The list has no header row to click, so sort key and direction live in the filter bar.
