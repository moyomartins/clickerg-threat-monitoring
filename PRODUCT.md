...
- Two levels: a **visitor list** — in Replay, a grid of cards rather than a table, each card reading verdict (IP and status) → plain-language reason → supporting facts (location, visit count, paid mix, spend, time-ago) → the drawn replay of the visitor's most recent reporting arrival — and a **visitor detail**, whose hero is a single bordered enclosure rather than a replay beside a summary: border weight and style state the verdict directly (2px ink blocked, dashed judgement call, dotted incomplete), the replay sits inside that border in its own column, one reason and the decisive moment sit beside the financial outcome, and visit counts, dates, confidence and exclusion state share one journey row beneath — then every arrival replayed in a horizontally scrolling strip with per-arrival signals and running confidence, and each card's plain-language explanation disclosed contextually on hover, focus, or tap rather than stacked in a block beneath the strip.
...
- **Each arrival explains itself, on demand.** The per-visit explanation — what happened, which signals moved confidence which way, the effect on the running total, whether it was the decisive arrival, cost or source — is disclosed by that arrival's own card (hover, keyboard focus, or tap), not read from a separate block beneath the strip. One generator (`src/arrival.ts`) produces the sentence the card's signals, the hero's reason, and this disclosure all agree with; there is exactly one explanation per visit, never two independently-worded ones.
...
- Working code is itself evidence: `README.md`, the `@clickerg/ui` Storybook, and a 102-test suite covering the scoring model, the hero's derived copy, the per-arrival explanation, and every story, with axe running at `error` severity so an accessibility violation fails the build.
...
Two real defects were found and fixed while rebuilding the detail hero, not just avoided: the page had no `<main>` landmark, and the replay's click tally carried an `aria-label` on a role-less `<span>`, which is invalid — it now carries `role="img"`. The detail page ran a live axe audit clean (zero violations) across all three pinned cases — blocked, judgement call, and incomplete — after both fixes landed.

**Known gap, not yet fixed:** a live axe run against the assembled visitor list finds `aria-allowed-role` on every card (`<article role="button">`, `role="button"` is not allowed on `<article>`) — 53 nodes, `minor` impact. The Storybook suite does not catch it because no story assembles a full card in that exact markup. Recorded here rather than silently left, since "enforced in the build" should mean the live surfaces, not only the isolated stories. The fix is a one-word tag or role change; it has been flagged but is out of scope for the work done so far, since the list card's DOM was explicitly out of scope for the tasks that touched this screen.

A real defect was found and fixed while building the per-arrival contextual explanation, caught by writing exhaustive test coverage over every generated visit rather than by eyeballing a handful of cards: `src/data/mock.ts`'s `clean-organic` archetype rolled `converted` and `conversionValueGbp` as two independent random draws, so a visit could convert with no recorded value — it rendered as the literal string "£undefined of actual revenue" in the explanation. Fixed by making it one decision, matching the pattern the `normal-prospect` archetype already used correctly.

**The arrival strip wraps, not scrolls, in the current build.** `.cg-journey` uses `grid-template-columns: repeat(5, minmax(150px,1fr))`, which wraps a journey past five visits onto additional rows. A same-session attempt to make it scroll instead (`grid-auto-flow: column`), to match the Strip-Scrolls-Not-Wraps Rule below, was reverted at the user's explicit direction as an unauthorized layout change outside that task's scope. The rule and the implementation now disagree; resolving that is future work, not a standing decision either way.
